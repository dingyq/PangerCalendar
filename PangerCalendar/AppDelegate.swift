...
//            print("result is \(response) error is \(error)")
            if error == nil {
                if response != nil {
//                    let result = (response!["result"])! as Array
...
//        application.registerUserNotificationSettings(UIUserNotificationSettings(types: .sound | .alert | .badge, categories: nil)
        ...
        let localNotify = launchOptions?[UIApplicationLaunchOptionsKey.localNotification]
        if localNotify != nil {
            let infoDic = (localNotify as! UILocalNotification).userInfo
            application.cancelLocalNotification(localNotify as! UILocalNotification)
            debugPrint(infoDic!.description)
        }
    ...
    func application(_ application: UIApplication, didReceive notification: UILocalNotification) {
        let infoDic = notification.userInfo
        debugPrint(infoDic!.description)
        application.cancelLocalNotification(notification)
        let alert = UIAlertView(title: "\(infoDic!["title"]!)", message: "", delegate: nil, cancelButtonTitle: "确定")
        alert.show()
    }
